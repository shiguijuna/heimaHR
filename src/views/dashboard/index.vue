...
export default {
  ...
  mounted() {
    ...
    this.socialDom = echarts.init(this.$refs.social)
    this.providentDom = echarts.init(this.$refs.provident)
  },
  methods: {
    MateData(val) {
      // 进行数据的截取
      ...
      return `<span style='color: #8a97f8;'>${name}</span> ${val}`
    }
  }
}
</script>
